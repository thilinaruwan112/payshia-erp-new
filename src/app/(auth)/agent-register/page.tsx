...
type AgentRegisterFormValues = z.infer<typeof agentRegisterFormSchema>;
...
export default function AgentRegisterPage() {
  ...
  const form = useForm<AgentRegisterFormValues>({
    resolver: zodResolver(agentRegisterFormSchema),
    defaultValues: {
        email: '',
        user_name: '',
        password: '',
  ...
  async function onSubmit(data: AgentRegisterFormValues) {
    setIsLoading(true);
    ...
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error(errorData.message || 'Failed to create agent account. Please try again.');
        }

        toast({
            title: 'Agent Account Created!',
            description: 'The agent account has been successfully created. Please ask the agent to log in.',
        });
        router.push('/login');
      ...
      <Card className="w-full max-w-2xl">
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)}>
            <CardHeader>
              <CardTitle className="text-xl">Agent Registration</CardTitle>
              <CardDescription>
                Enter the information to create a new agent account.
              </CardDescription>
            </CardHeader>
            <CardContent>
