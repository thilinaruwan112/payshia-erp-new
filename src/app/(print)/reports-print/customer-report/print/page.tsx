...
function PrintViewContent() {
  ...
  useEffect(() => {
    async function fetchData() {
        ...
        try {
             const [customersRes, companyRes] = await Promise.all([
  ...
  useEffect(() => {
    if (!isLoading && customers.length > 0) {
      document.title = `Customer Master Report`;
    ...
    <div className="bg-white text-black font-sans text-sm w-[210mm] min-h-[297mm] shadow-lg print:shadow-none p-8">
        <header className="flex justify-between items-start pb-4 border-b">
            ...
            <div className="text-right">
                <h2 className="text-2xl font-bold uppercase">Customer Master Report</h2>
            </div>
        </header>
                ...
                <tbody>
                    {customers.map((customer) => (
                        <tr key={customer.customer_id} className="border-b">
                            <td className="p-2 border border-gray-300">{customer.customer_first_name} {customer.customer_last_name}</td>
                            <td className="p-2 border border-gray-300">{customer.phone_number}</td>
                            <td className="p-2 border border-gray-300">{customer.email_address}</td>
                            <td className="p-2 border border-gray-300">{customer.address_line1}, {customer.city}</td>
                        </tr>
                    ))}
                </tbody>
            </table>
