...
export default function SizesPage() {
  ...
  useEffect(() => {
    async function fetchSizes() {
      ...
      setIsLoading(true);
      try {
        const response = await fetcher(`${process.env.NEXT_PUBLIC_API_BASE_URL}/sizes/filter/company?company_id=${company_id}`);
        if (!response.ok) {
  ...
  const handleDelete = async () => {
    ...
    try {
        const response = await fetcher(`${process.env.NEXT_PUBLIC_API_BASE_URL}/sizes/${selectedSize.id}`, {
            method: 'DELETE',
        });
        if (!response.ok) {
