...
export default function SizesPage() {
  ...
  useEffect(() => {
    async function fetchSizes() {
      ...
      setIsLoading(true);
      try {
        const response = await fetcher(`https://server-erp.payshia.com/sizes/filter/company?company_id=${company_id}`);
        if (!response.ok) {
  ...
  const handleDelete = async () => {
    ...
    try {
        const response = await fetcher(`https://server-erp.payshia.com/sizes/${selectedSize.id}`, {
            method: 'DELETE',
        });
        if (!response.ok) {
